...
jest.unstable_mockModule("../../database/conn.js", async () => ({
  User: {
    findOne: jest.fn(),
    create: jest.fn(),
  },
}));
jest.unstable_mockModule("../../services/authService.js", async () => ({
  verifyPsw: jest.fn(),
  createToken: jest.fn(),
  hashPsw: jest.fn(),
  verifyToken: jest.fn(),
...
describe("Auth Controller Test - Register handler", () => {
  it("should return sucess with valid request", async () => {
    ...
    it("should return error with invalid request", async () => {
      const expectedError = new Error("Email não fornecido");
